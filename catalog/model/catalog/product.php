...
class ModelCatalogProduct extends Model {
    ...
    public function getCategoryType($product_id) {
        $sql = "SELECT p.category_id AS category_id, cd.meta_title AS meta_title FROM " . DB_PREFIX . "product_to_category p";
        $sql .= " INNER JOIN ".DB_PREFIX."category_description cd ON p.category_id = cd.category_id";
        ...
        if (count($query->rows) > 0) {
            foreach ($query->rows as $category) {
                if ($category['meta_title'] == 'category_men' || $category['meta_title'] == 'category_women') {
                    $type = 'shoes';
                } elseif ($category['meta_title'] == 'category_bag') {
                    $type = 'bag';
                }
            }
        }
	...
	public function getBestSellerProductsByCategory($category_id) {
		$productInCategory = $this->db->query("SELECT `p`.`product_id` AS id FROM `".DB_PREFIX."product_to_category` `p` WHERE `p`.`category_id`=".(int)$category_id." ORDER BY `p`.`product_id` DESC");
		$listProduct = '(';
		if ($productInCategory->num_rows) {
			foreach ($productInCategory->rows as $key => $result) {
				$listProduct .= ($key == ($productInCategory->num_rows - 1) ? $result['id'] : $result['id'].', ');
			}
		} else {
			return array();
		}
    ...
    public function getYouCanLikeProduct($limit = 3) {
        $productIds = $this->db->query("SELECT product_id AS id FROM ".DB_PREFIX."product p WHERE p.status = '1' AND p.date_available <= NOW() ORDER BY id");
        $idList = array();
        foreach ($productIds->rows as $currId) {
            array_push($idList, $currId['id']);
        }
        $numProduct = count($idList);
        $n = 0;
        $notFound = true;
        $listProduct = '(';
        while ($notFound && $n < 3) {
